...
class ReloadTest(BaseTest):
    ...
    def __init__(self):
        ...
        self.arp_loopback_ping_pkts = 10
        self.nr_pc_pkts = 100
        self.nr_tests = 3
        ...
        self.loopback_state = StateMachine('init')
    ...
    def generate_arp_loopback_ping_packets(self):
        self.arp_loopback_ping_packets = []
        dut_lo_ipv4 = self.lo_prefix.split('/')[0]
        ...
        for src_port in self.active_port_indices if self.is_dualtor else self.vlan_host_ping_map:
            ...
            self.arp_loopback_ping_packets.append((src_port, bytes(packet), ping_dut_exp_packet))
    ...
    def log_loopback_state_change(self, reachable):
        old = self.loopback_state.get()
        ...
        self.loopback_state.set_flooding(flooding)

        if old != state:
            self.log("Loopback state transition from %s to %s" %
                     (old, state))
            self.loopback_state.set(state)
    ...
    def reachability_watcher(self):
        ...
        while self.watching:
            ...
            self.log('Reachability watcher - checking Loopback IP')
            total_rcv_pkt_cnt = self.pingDut()
            reachable = total_rcv_pkt_cnt > 0 and total_rcv_pkt_cnt > self.arp_loopback_ping_pkts * 0.7
            partial = total_rcv_pkt_cnt > 0 and total_rcv_pkt_cnt < self.arp_loopback_ping_pkts
            flooding = reachable and total_rcv_pkt_cnt > self.ping_dut_pkts
            self.log_cpu_state_change(reachable, partial, flooding)
    ...
    def arpLoopbackPing(self):
        total_rcv_pkt_cnt = 0
        packets = random.choices(self.arp_loopback_ping_packets, k=self.arp_loopback_ping_pkts)
        for i in packets:
        ...
        self.log("Send %5d Received %5d arp loopback ping" %
                 (self.arp_loopback_ping_pkts, total_rcv_pkt_cnt), True)
        return total_rcv_pkt_cnt
